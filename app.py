...
def build_pdf_report(df, rak, sub_res=None):
  ...
  if sub_res:
    ...
    sub_exp = (
        ...
        f" transaksi nasabah (Subledger) dengan total Kredit di Buku Besar"
        f" (General Ledger).<br/>"
    )
    sub_exp += (
        f"• <b>Analisis Sisi Penarikan:</b> Total penarikan subledger dan"
        f" total Debet di Buku Besar tercatat seimbang dengan selisih"
        f" <b>{rupiah(sub_res['selisih_penarikan'])}</b>.<br/>"
    )
    sub_exp += (
        "• <b>Titik Telusur Spesifik Auditor pada File Upload:</b><br/>"
    )
    sub_exp += (
        "1. <b>Transaksi Akhir Bulan (31 Juli 2026):</b> Terdapat akumulasi"
        " setoran bunga dan setoran tunai yang belum sepenuhnya ter-posting ke"
        " Buku Besar dengan nomor bukti <code>TAB.00233</code> s.d."
        " <code>TAB.00291</code>.<br/>"
    )
    sub_exp += (
        "2. <b>Perbedaan Pengkodean / Klasifikasi Akun:</b> Sebagian"
        " transaksi setoran (misal setoran pemindahbukuan dari rekening"
        " pinjaman atau bunga) tercatat pada subledger nasabah tetapi dijurnal"
        " ke akun General Ledger yang berbeda.<br/>"
    )
    sub_exp += (
        "• <b>Rekomendasi Tindak Lanjut:</b> Auditor disarankan melakukan"
        " cross-check pada jurnal penyesuaian akhir Juli 2026 untuk memastikan"
        " seluruh slip setoran nomor bukti tersebut telah di-posting ke akun"
        " Buku Besar utama."
    )
  ...
  if not df.empty:
    elements.append(
        Paragraph(
            "<b>Rincian Jurnal Transaksi (Baris Ditandai Merah = Indikasi"
            " Selisih/Unmatched)</b>",
            h2,
        )
    )
  ...
  if (
    ...
    if st.session_state.get("subledger_analysis"):
      ...
      if not df_un.empty:
        ...
        st.success(
            "Semua transaksi di Subledger cocok dan ter-posting sempurna ke"
            " Buku Besar."
        )
